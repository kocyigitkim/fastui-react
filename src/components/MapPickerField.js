...
function LocationMarker(props) {
	...
	useMapEvents({
		click(evt) {
			if (props.onChange) props.onChange(evt.latlng);
			setPosition(evt.latlng);
		}
	});
	var latlng = props.value || position;
	if (latlng) latlng = new LatLng(latlng.lat, latlng.lng);

	return latlng === null ? null : (
		<Marker icon={L.icon({
			iconUrl: GoogleMarkerBase64,
			iconRetinaUrl: GoogleMarkerBase64,
			iconSize: [21.333, 37.218],
			iconAnchor: [10.5, 34.892],
			popupAnchor: [0, -34.892],
			shadowUrl: null,
			shadowSize: null,
			shadowAnchor: null,
		})} position={latlng}>
...
export default class MapPickerField extends CustomField {
	isLocatedWhenInit = false;
	constructor(props) {
		super(props);
		/** @type {Map} */
		this.mapRef = null;
	...
	grid(props) {
		if (props.value && props.value.lat && props.value.lng) {
			return (
				<div>
					<button type="button" className="btn btn-dark" onClick={() => {
						window.open("https://maps.google.com/?q=" + props.value.lat + "," + props.value.lng, '_blank');
					}}>
			...
			return (
				<div>
					<i className="bi bi-pin-map-fill text-danger" style={{ fontWeight: 'bold' }}></i>
				</div>
			)
		}
	}
	...
	componentDidUpdate() {
		if (this.mapRef && this.props.value && this.isLocatedWhenInit === false) {
			this.isLocatedWhenInit = true;
			this.gotoLocation.call(this);
		}
	}
	gotoLocation() {
		if (this.mapRef && this.props.value) {
			this.mapRef.flyTo(this.props.value, 18);
		}
	}
	form() {
		...
		const _this = this;
		const tileLayerURL = props.tileLayerURL || "http://mt1.google.com/vt/lyrs=y&x={x}&y={y}&z={z}";
		const latlng = props.value || null;
		const MapReferenceComponent = function (props) {
			_this.mapRef = useMap();
			return <div></div>;
		};
		return (
			<ReactBridge ref={r => this.mapBridge = r}>
				<div style={{ display: 'flex' }}>
					<div style={{ flex: 1 }}> {title && <div className="form-label">{translated}</div>}</div>
					<div>
						<Field type="button" onClick={this.gotoLocation.bind(this)} title="MAP.PICKER.LOCATE"></Field>
					</div>
				</div>
				<MapContainer attributionControl={false} placeholder={(<div></div>)} center={this.props.center || [0, 0]} zoom={this.props.zoom || 3} scrollWheelZoom={this.props.scrollWheelZoom} style={this.props.containerStyle}>
					<TileLayer
						attribution=''
						url={tileLayerURL}
					/>
					<MapReferenceComponent />
					<LocationMarker value={latlng} onChange={this.props.onChange}></LocationMarker>
				</MapContainer>
				{latlng && latlng.lat && latlng.lng && (<div>
					<label style={{ fontWeight: 'bold', fontSize: '0.8rem', marginTop: 5 }}>{latlng.lat},{latlng.lng}</label>
				</div>)}
